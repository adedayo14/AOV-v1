(function() {
  ...
  class UpCartDrawer {
    ...
    closeDrawer() {
      ...
      // Reset animation state so next open re-animates
      const drawer = container.querySelector('.upcart-cart');
          ...
          if (isAddToCart && resp.ok) {
            ...
            setTimeout(async () => {
              try {
                await this.fetchCart();
                this.openDrawer();
                // Update content after opening (the open state will be preserved)
                setTimeout(() => this.updateDrawerContent(), 10);
              } catch (e) {
