(function() {
  ...
  class UpCartDrawer {
    ...
    async setup() {
      ...
      // Always create our cart drawer and implement aggressive hijacking
    ...
    interceptAllCartClicks() {
      ...
      document.addEventListener('click', (e) => {
        ...
        if (target && target.href && target.href.includes('/cart')) {
          console.log('🛒 Intercepted cart link:', target.href);
          e.preventDefault();
          e.stopPropagation();
          this.openDrawer();
          return false;
    ...
    overrideShopifyCartFunctions() {
      ...
      const originalFunctions = {
        ...
        openCartDrawer: window.openCartDrawer,
        toggleCartDrawer: window.toggleCartDrawer,
        showCartDrawer: window.showCartDrawer,
        openMiniCart: window.openMiniCart
      };
          ...
          this.addEventListener('load', () => {
            if (this.status === 200) {
              window.UpCart.fetchCart().then(() => {
                window.UpCart.updateCartDrawerContent();
