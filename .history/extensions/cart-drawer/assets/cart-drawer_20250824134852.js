(function() {
  ...
  class UpCartDrawer {
    constructor(settings) {
      console.log('UpCart: Constructor called with settings:', settings);
      this.settings = settings || window.UpCartSettings;
      console.log('UpCart: Final settings:', this.settings);
      this.cart = null;
      this.isOpen = false;
      this.init();
    }
    ...
    async setup() {
      console.log('UpCart: Setup called');
      console.log('UpCart: Current pathname:', window.location.pathname);
      console.log('UpCart: showOnlyOnCartPage:', this.settings.showOnlyOnCartPage);
      console.log('UpCart: isCartPage:', this.isCartPage());
      
      // Check if we should only show on cart page
      if (this.settings.showOnlyOnCartPage && !this.isCartPage()) {
        console.log('UpCart: Not on cart page, exiting setup');
        return; // Don't initialize if not on cart page
      }
      
      console.log('UpCart: Proceeding with setup');
      
      // Fetch initial cart
      await this.fetchCart();
      console.log('UpCart: Cart fetched:', this.cart);
      
      // Create UI elements
      if (this.settings.enableStickyCart) {
        console.log('UpCart: Creating sticky cart');
        this.createStickyCart();
      }
      
      if (this.settings.enableFreeShipping) {
        console.log('UpCart: Creating free shipping bar');
    ...
    isCartPage() {
      ...
      console.log('UpCart: Cart page detection:', {
        pathname,
        isCart,
        bodyClasses: document.body.className,
        hasCartClass: document.querySelector('[class*="cart"]') !== null
      });
