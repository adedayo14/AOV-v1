  ...
  class UpCartDrawer {
    ...
    async setup() {
      ...
      if (this.settings.showOnlyOnCartPage && !this.isCartPage()) {
        ...
        // Set up cart drawer detection
        this.watchForCartDrawers();
        return;
      }
      ...
      if (this.settings.enableStickyCart) {
        ...
        console.log('🛒 Sticky cart disabled in settings');
      }
      ...
      if (this.settings.enableFreeShipping) {
        ...
        console.log('🛒 Free shipping disabled in settings');
      }
    ...
    watchForCartDrawers() {
        ...
        mutations.forEach((mutation) => {
          mutation.addedNodes.forEach((node) => {
            if (node.nodeType === Node.ELEMENT_NODE) {
              ...
              if (this.isCartDrawerElement(node)) {
                console.log('🛒 Cart drawer detected!', node);
                this.injectIntoCartDrawer(node);
              }
              
              // Check children for cart drawers
              const cartDrawers = node.querySelectorAll ? 
                node.querySelectorAll('[data-cart-drawer], .cart-drawer, #cart-drawer, .drawer--cart, [id*="cart-drawer"], [class*="cart-drawer"]') : [];
              
              cartDrawers.forEach(drawer => {
                console.log('🛒 Cart drawer found in children!', drawer);
      ...
      // Also check for existing cart drawers
      const existingDrawers = document.querySelectorAll('[data-cart-drawer], .cart-drawer, #cart-drawer, .drawer--cart, [id*="cart-drawer"], [class*="cart-drawer"]');
      existingDrawers.forEach(drawer => {
        console.log('🛒 Existing cart drawer found!', drawer);
        this.injectIntoCartDrawer(drawer);
      });
    }
    ...
    injectIntoCartDrawer(drawerElement) {
      ...
      console.log('🛒 Injecting UpCart into cart drawer...');
      ...
      upCartSection.innerHTML = `
        <div class="upcart-drawer-enhancement">
          <div class="upcart-free-shipping-progress" style="padding: 16px; border-bottom: 1px solid #eee;">
            ${this.getFreeShippingHTML()}
          </div>
          <div class="upcart-upsells-section" style="padding: 16px;">
      ...
      const footer = drawerElement.querySelector('.cart-footer, .drawer__footer, [class*="footer"], .cart-actions, .drawer-actions');
      if (footer) {
      ...
      console.log('🛒 UpCart successfully injected into cart drawer!');
    }
