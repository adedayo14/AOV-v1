(function() {
  ...
  class UpCartDrawer {
    ...
    async setup() {
      ...
      // Create the main drawer UI
      this.createDrawer();
      
      // Create sticky cart if enabled and conditions are met
      if (this.settings.enableStickyCart) {
        if (this.settings.showOnlyOnCartPage && !this.isCartPage()) {
          console.log('🛒 Sticky cart not shown: not on cart page');
        } else {
      ...
      // Set up cart functionality (interception, etc.)
  ...
  // Log anomalies (like extremely large thresholds) to help merchant debug settings
  this.detectAnomalies();
  // Start observer to guard against external scripts blanking our drawer
  this.startBlankGuard();
    }
    ...
    createStickyCart() {
      ...
      const existing = document.getElementById('upcart-sticky-cart');
    ...
    createDrawer() {
          ...
          <div class="upcart-cart">
            ...
            <div class="upcart-progress-section">
              <div class="upcart-free-shipping-bar">
                <div class="upcart-free-shipping-text">🚚 You're $${this.formatMoneyRaw(Math.max(0, this.getFreeShippingThresholdCents() - totalPrice)).replace('$','')} away from free shipping!</div>
                <div class="upcart-progress-bar">
                  <div class="upcart-progress-fill" style="width: ${Math.min((totalPrice / this.getFreeShippingThresholdCents()) * 100, 100)}%"></div>
                </div>
                <div class="upcart-progress-remaining">
                  ${totalPrice >= this.getFreeShippingThresholdCents() ? '🎉 You qualify for free shipping!' : `Add $${this.formatMoneyRaw(Math.max(0, this.getFreeShippingThresholdCents() - totalPrice)).replace('$','')} more for free shipping`}
                </div>
              </div>
            </div>
            ` : ''}
      ...
      console.log('🛒 Drawer HTML injected. Length:', container.innerHTML.length);
      // Debug: verify critical sub-elements exist
      ...
      requiredSelectors.forEach(sel => {
        ...
        console.log('🛒 Render check', sel, 'found =', !!found);
      });
  ...
  // Schedule a follow-up validation to ensure content wasn't wiped by theme scripts
  setTimeout(() => this.ensureDrawerRendered('post-create timeout 1'), 150);
  setTimeout(() => this.ensureDrawerRendered('post-create timeout 2'), 500);
  setTimeout(() => this.ensureDrawerRendered('post-create timeout 3'), 1500);
    }

    updateDrawerContent() {
      try {
        console.log('🛒 updateDrawerContent() start. Cart present:', !!this.cart, 'item_count:', this.cart && this.cart.item_count);
        ...
        if (this.cart.items.length === 0) {
          ...
          // Guard each item render to avoid one bad item blanking everything
          const itemHTML = [];
          for (const item of this.cart.items) {
            try {
              const safeImg = item.image ? `<img src="${item.image}" alt="${(item.title||'').replace(/"/g,'&quot;')}" loading="lazy">` : '';
              itemHTML.push(`
              <div class="upcart-item" data-variant-id="${item.variant_id}" data-key="${item.key}">
                <div class="upcart-item-image">${safeImg}</div>
              ...
              console.warn('🛒 Error rendering item, skipping:', item, itemErr);
            }
          }
          itemsContainer.innerHTML = itemHTML.join('');
        }
        ...
        console.error('🛒 updateDrawerContent error – creating fallback skeleton', err);
        const itemsContainer = document.querySelector('.upcart-items');
        if (itemsContainer) {
          itemsContainer.innerHTML = `<div class="upcart-error" style="padding:16px; text-align:center;">
            <p style="margin:0 0 8px; font-weight:600;">Cart temporarily unavailable</p>
            <p style="margin:0; font-size:12px; opacity:0.7;">Retrying...</p>
          </div>`;
        }
        setTimeout(() => {
          this.fetchCart().then(() => this.updateDrawerContent());
        }, 800);
      }
    }
