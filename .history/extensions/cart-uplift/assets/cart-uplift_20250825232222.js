(function() {
  ...
  getCartItemsHTML() {
    ...
    getRecommendationsHTML() {
      // Hide entire section if no recommendations are available
      if (!this.recommendations || this.recommendations.length === 0) {
        // Only show if recommendations are still loading
        ...
        // If loaded but no recommendations, return empty string (hide section)
        return '';
      }
    ...
    getRecommendationItems() {
      // This method is only called when recommendations exist, so no need for empty checks
      const layout = this.settings.recommendationLayout || 'column';
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (this.recommendations && this.recommendations.length > 0) {
            ...
            // Auto-close recommendations if all products have been added
            if (this.recommendations.length === 0) {
              console.log('🛒 All recommendations added, auto-closing section');
              setTimeout(() => {
                const container = document.getElementById('cartuplift-app-container');
                if (container) {
                  const recommendations = container.querySelector('.cartuplift-recommendations');
                  const toggleButton = container.querySelector('.cartuplift-recommendations-toggle');
                  if (recommendations && !recommendations.classList.contains('collapsed')) {
                    recommendations.classList.add('collapsed');
                    
                    // Update arrow direction to point down (collapsed state)
                    if (toggleButton) {
                      const arrow = toggleButton.querySelector('svg path');
                      if (arrow) {
                        arrow.setAttribute('d', 'M12 6L8 10L4 6');
