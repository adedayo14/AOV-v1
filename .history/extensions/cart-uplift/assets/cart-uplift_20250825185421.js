(function() {
  ...
  class CartUpliftDrawer {
    constructor(settings) {
      ...
      this.settings.enableApp = Boolean(this.settings.enableApp !== false); // Default to true
    ...
    openDrawer() {
      ...
      // Be open immediately so 'click-out' works right away
      this.isOpen = true;
      this._isAnimating = false;
    }

    closeDrawer() {
      if (this._isAnimating) return; // allow close even if isOpen hasn't flipped yet
      ...
      // Immediately disable backdrop hitbox so nothing blocks clicks
      const backdrop = container.querySelector('#cartuplift-backdrop');
      if (backdrop) {
      ...
      // Start close by removing active (CSS handles the slide-out)
      container.classList.remove('active');

      const finishClose = () => {
        container.style.display = 'none';

        // Only undo OUR flags/styles
        ...
        // Ensure our backdrop cannot intercept anything
        const bd = container.querySelector('#cartuplift-backdrop');
        if (bd) {
          bd.style.display = 'none';
          bd.style.opacity = '0';
          bd.style.visibility = 'hidden';
          bd.style.pointerEvents = 'none';
          bd.style.zIndex = ''; // clear any leftovers
        }
      ...
      // Safety timer; don't rely on animation events
      setTimeout(finishClose, 350);
    }

    forceCleanThemeArtifacts() {
      console.log('🛒 Minimal cleanup (ours only)');
      // Only touch our own global flags
      document.documentElement.classList.remove('js-drawer-open'); // some themes add this; harmless to clear
      document.body.classList.remove('js-drawer-open');

      // Never reset filters/pointer-events on arbitrary elements here.
      // Leave themes alone unless you have a specific, tested selector.
    }
    ...
    restorePageInteraction() {
      console.log('🛒 Restoring interaction (scoped to Cart Uplift)');

      // Only remove our own flags and styles
      document.documentElement.classList.remove('cartuplift-drawer-open');
      document.body.classList.remove('cartuplift-drawer-open');
      document.documentElement.style.overflow = '';
      document.body.style.overflow = '';

      // Our container/backdrop back to inert
      const container = document.getElementById('cartuplift-app-container');
      if (container) {
        container.classList.remove('active');
        container.style.display = 'none';
