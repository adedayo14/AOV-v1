(function() {
  ...
  class CartUpliftDrawer {
      ...
      window.fetch = async (...args) => {
        ...
        try {
          ...
          if (isAddToCart && resp.ok && !this._isAnimating) { // prevent during animations
            console.log('🛒 Add to cart detected, auto-open setting:', this.settings.autoOpenCart);
            ...
            setTimeout(async () => {
              try {
                ...
                if (this.settings.autoOpenCart && !this.isOpen && !this._isAnimating) {
                  ...
                  // AGGRESSIVE pre-cleanup before opening
                  this.forceCleanThemeArtifacts();
                  
                  // Clean again just before opening
                  this.forceCleanThemeArtifacts();
                  
                  this.openDrawer();
                  
                  // Clean again after opening to catch any theme interference
                  setTimeout(() => this.forceCleanThemeArtifacts(), 100);
                  setTimeout(() => this.forceCleanThemeArtifacts(), 300);
                  setTimeout(() => this.forceCleanThemeArtifacts(), 500);
                } else {
    ...
    detectAndRemoveBlur() {
      // Check for elements with blur/filter styles
      const elementsWithBlur = document.querySelectorAll('*:not(#cartuplift-app-container):not(#cartuplift-app-container *)');
      ...
      elementsWithBlur.forEach(el => {
        ...
        if (hasFilter || hasBackdropFilter) {
          console.log('🛒 Blur detected on element, removing:', el, { 
