(function() {
  ...
  gridItem.dataset.price = this.formatMoney(normalizePriceToCents(product.priceCents || 0));
        ...
        const titleEl = gridItem.querySelector('.cartuplift-grid-title');
        if (titleEl) titleEl.textContent = this.escapeHtml(product.title);
      }
    }
  ...
  black: '#000', white: '#fff', ivory: '#fffff0', beige: '#f5f5dc', cream: '#fffdd0', offwhite: '#f8f8f8', 'off white': '#f8f8f8',
        ...
        orange: '#fb923c', coral: '#ff7f50', peach: '#ffdab9',
        yellow: '#facc15', gold: '#d4af37', mustard: '#e1ad01',
        green: '#22c55e', olive: '#556b2f', mint: '#98ff98', teal: '#14b8a6',
        blue: '#3b82f6', navy: '#001f3f', sky: '#38bdf8', cobalt: '#0047ab',
        purple: '#a855f7', violet: '#8b5cf6', lavender: '#e6e6fa', lilac: '#c8a2c8',
        brown: '#8b4513', tan: '#d2b48c', chocolate: '#7b3f00'
      };
      if (named[v]) return named[v];
      // Try to parse hex codes (#fff or #ffffff or with alpha)
    ...
    getVariantOptionsHTML(item) {
      ...
      if (item.variant_title && item.options_with_values && Array.isArray(item.options_with_values)) {
        const parts = item.options_with_values
          ...
          .map(opt => renderPart(opt.name, opt.value));
        return parts.join('');
      }
      ...
      if (Array.isArray(item.variant_options) && Array.isArray(item.options)) {
        item.variant_options.forEach((optValue, index) => {
          ...
          variants.push(renderPart(optName, optValue));
        });
      }
  ...
  if (window.CartUpliftSettings && !window.cartUpliftDrawer) {
    ...
    if (window.CartUpliftSettings.enableSmartBundles && !window.cartUpliftBundleRenderer) {
  ...
  // Listen for settings loaded event (from async loading)
  document.addEventListener('cartuplift:settings-loaded', function(e) {
    if (!window.cartUpliftDrawer && e.detail) {
      window.cartUpliftDrawer = new CartUpliftDrawer(e.detail);
      
      // Initialize bundle renderer if bundles are enabled
      if (e.detail.enableSmartBundles && !window.cartUpliftBundleRenderer) {
        const script = document.createElement('script');
        const assetUrl = (window.CartUpliftAssets && window.CartUpliftAssets.bundleRenderer) || '/apps/cart-uplift/assets/bundle-renderer.js';
        script.src = assetUrl;
        script.onload = function() {
          if (window.BundleRenderer) {
            window.cartUpliftBundleRenderer = new window.BundleRenderer(e.detail);
          }
        };
        document.head.appendChild(script);
      }
